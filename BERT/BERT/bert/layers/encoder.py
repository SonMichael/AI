...
class Encoder(tf.keras.layers.Layer):
    ...
    def call(self, input_ids, segment_ids, is_train, mask):
        ...
        pos_expand_dim = tf.expand_dims(pos, axis=0)#(1, max_length)
        pos_broadcast = tf.broadcast_to(pos_expand_dim, [batch_size, len_q])#(batch_size, max_length)
        encoder_out = self.tok_embed(input_ids) + self.pos_embed(pos_broadcast) + self.seg_embed(segment_ids)#(batch_size, max_length, d_model)
